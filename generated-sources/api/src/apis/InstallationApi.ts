...
export interface InstallationApiInterface {
    ...
    /**
     * Update an installation config. Use `updateMask` to specify which fields to update; unmasked fields are left unchanged. Each masked field is **replaced**, so provide the complete value for each field you include.  Updates to subscribe config (`config.content.subscribe.objects` or `config.content.subscribe.objects.<objectName>`) trigger a subscription change in the provider\'s system, which typically takes 1-2 minutes but may take up to 10 minutes. 
