...
export function determineModule(
  ...
  if (!moduleToUse) {
    return {
      module: "",
      error: `Provider "${providerInfo?.name}" has modules but no valid module could be determined. Integration must specify a module or provider must have a defaultModule.`,
    };
  }
