...
export function FieldDefaultValueTable({
  ...
  const selectedValueDefaultsMap = useMemo(
    () => writeObject?.selectedValueDefaults || {},
    [writeObject],
  );

  const onAddDefaultValue = useCallback(
    (field: string, fieldDisplayName: string, defaultValue: string) => {
      setValueDefaultWriteField(
        selectedObjectName || "",
        objectName,
        field,
        defaultValue,
  ...
  const defaultValueList: FieldDefaultValue[] = Object.keys(
    selectedValueDefaultsMap,
  ).map((field) => ({
    field,
    defaultValue: selectedValueDefaultsMap[field]?.value?.toString(),
    // consider memoizing if performance is an issue.
    fieldDisplayName: readAction
      ? getDisplayNameFromField(field, objectName, readAction)
      : field,
  }));
  ...
  return (
    <>
      ...
      >
        {defaultValueList.map(
          ({ field, fieldDisplayName, defaultValue: df }) => (
            <div
              key={`${field}-${df}`}
      ...
      <NewDefaultValueUI
        ...
        onAddDefaultValue={onAddDefaultValue}
      />
      <div
        style={{
          textAlign: "right",
