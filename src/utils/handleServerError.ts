...
import { getApiProblemParts, serializeApiProblem } from "src/utils/apiProblem";
  ...
  if (error instanceof ResponseError) {
    ...
    try {
      ...
      if (!errorMsg) {
        ...
        if (errorBody?.remedy) {
          console.error("[Remedy]", errorBody.remedy);
        }
      }

      const combinedErrorMessage = serializeApiProblem(
        getApiProblemParts(errorBody),
      );
